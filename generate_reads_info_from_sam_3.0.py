...
def decide_ref_or_alt():
    '''
    in this function, we match 3 different mode.
    for example:
        125M
        10M5N115M
        10M5N5M10N110M
    '''
    if re.search('^(\d+)M$',reads[5]):
        for i in range(reads_length):
            sub_reads_info=reads[9][i]
            decide_ref_or_alt_sub(i,sub_reads_info)
    elif re.search('^(\d+)M(\d+)N(\d+)M$',reads[5]):
        ...
        for i in range(int(pos_number[0])+int(pos_number[1]),int(pos_number[0])+int(pos_number[1])+int(pos_number[2])):
            ...
            decide_ref_or_alt_sub(i,sub_reads_info)
    elif re.search('^(\d+)M(\d+)N(\d+)M(\d+)N(\d+)M',reads[5]):
        pos_number=re.findall('\d+',reads[5])
        for i in range(int(pos_number[0])):
            sub_reads_info=reads[9][i]
            decide_ref_or_alt_sub(i,sub_reads_info)
...
for reads in f2:
    reads=reads.split()
    if reads[2] in genome_dict and reads[5]!='*':
        decide_ref_or_alt()
        get_region()
    else:
        pass
...
for key,value in reads_ref.items():
    try:
        f3.write(str(key)+'\t'\
                ...
                +str(reads_region_start[key])+'\t'\
                +str(reads_region_end[key])+'\t'\
